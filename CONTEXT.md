...
6. **Final test metrics** (evaluated on the untouched original test set, not SMOTE-balanced, for fair comparison): LSTM+SMOTE — R2L recall 0.10 / F1 0.19, U2R recall 0.42 / F1 0.45, macro-avg F1 0.51. RF flat baseline scored macro-avg F1 0.34 for comparison.
7. **Artifacts**: `best_nslkdd_smote.keras`, `scaler_nslkdd.pkl`, `label_encoders_nslkdd.pkl` — all three required together at inference.
